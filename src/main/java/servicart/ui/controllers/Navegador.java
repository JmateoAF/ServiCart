...
import servicart.domain.services.admin.*;
import servicart.domain.services.cliente.*;
import servicart.ui.controllers.admin.*;
...
public class Navegador {
    ...
    public static Object crearControlador(Class<?> claseControlador) {
        ...
        if (claseControlador == PerfilClienteController.class)
            return new PerfilClienteController(perfilCliente, contratoCliente);

        if (claseControlador == LoginAdminController.class) return new LoginAdminController(loginAdmin);

        if (claseControlador == AdminUsuariosController.class)
            return new AdminUsuariosController(adminUsuarios, panelAdmin);
